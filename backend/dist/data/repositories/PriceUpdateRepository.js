...
export class PriceUpdateRepository extends BaseRepository {
    constructor() {
        super(...arguments);
        this.entityKey = 'price_update';
        this.entityName = 'PriceUpdate';
    }
    matchesSearchPattern(entity, pattern) {
        const searchPattern = pattern.toLowerCase();
        return (entity.source.toLowerCase().includes(searchPattern) ||
    ...
    async findByTokenAddress(tokenAddress, params) {
        try {
            ...
            logger.error('PriceUpdate token adres ile bulma hatası', { error: error.message, tokenAddress });
            throw error;
        }
    }
    // Find price updates by source
    async findBySource(source, params) {
        try {
            const indexKey = this.getIndexKey();
            const allIds = await redisDatabase.getClient().sMembers(indexKey);
            const matchingIds = [];
            ...
            logger.error('PriceUpdate kaynak ile bulma hatası', { error: error.message, source });
            throw error;
        }
    }
    ...
    async findByDateRange(startDate, endDate, params) {
        try {
            ...
            logger.error('PriceUpdate tarih aralığı ile bulma hatası', { error: error.message, startDate, endDate });
            throw error;
        }
    }
    ...
    async findWithFilters(filters, params) {
        try {
            ...
            logger.error('PriceUpdate filtre ile bulma hatası', { error: error.message, filters });
            throw error;
        }
    }
    ...
    async getLatestPrice(tokenAddress) {
        try {
            ...
            logger.error('En son fiyat bulma hatası', { error: error.message, tokenAddress });
            return null;
        }
    }
    // Get price history for a token
    async getPriceHistory(tokenAddress, limit = 100) {
        try {
            const indexKey = this.getIndexKey();
            const allIds = await redisDatabase.getClient().sMembers(indexKey);
            const matchingPriceUpdates = [];
            for (const id of allIds) {
                const priceUpdate = await this.findById(id);
                if (priceUpdate) {
                    if (tokenAddress) {
            ...
            logger.error('Fiyat geçmişi bulma hatası', { error: error.message, tokenAddress });
            return [];
        }
    }
    ...
    async getStats() {
        try {
            ...
            logger.error('PriceUpdate istatistik hatası', { error: error.message });
            return {
                total: 0,
                bySource: {},
                byToken: {},
    ...
    async cleanOldPriceUpdates(daysOld) {
        try {
            ...
            logger.error('Eski fiyat güncellemeleri temizleme hatası', { error: error.message, daysOld });
            return 0;
        }
    }
}
//# sourceMappingURL=PriceUpdateRepository.js.map